...
import { Save, LogOut, Upload, Plus, Trash2, Loader2, Image as ImageIcon, Video, Briefcase, User, PenTool, Layout, Type, Bold, Italic, Menu, X } from 'lucide-react';
    ...
    <div className="mb-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 border rounded-lg text-sm bg-white hover:bg-gray-50 shadow-sm transition active:scale-95">
          {uploading ? <Loader2 className="animate-spin h-4 w-4"/> : (type === 'video' ? <Video size={16}/> : <ImageIcon size={16}/>)}
          <span className="font-medium">{uploading ? '...' : (currentUrl ? 'Ganti File' : 'Upload File')}</span>
          <input type="file" className="hidden" accept={type === 'video' ? "video/*" : "image/*"} onChange={handleFileChange} disabled={uploading} />
        </label>
      </div>
      {currentUrl && <p className="mt-1 text-[10px] text-gray-400 truncate max-w-[200px]">{currentUrl.split('/').pop()}</p>}
    </div>
  );
};
                  ...
                  {field.type === 'media' ? (
                    <MediaUploader label={field.label} currentUrl={item[field.key]} type={field.mediaType || 'image'} onUpload={(url) => updateItem(idx, field.key, url)} />
                     ...
                     <div className="mt-2">
                       ...
                       <textarea 
                         className="w-full p-3 border rounded-lg text-sm bg-gray-50 focus:bg-white transition focus:ring-1 ring-black/10 outline-none" 
                         rows={3}
...
export default function AdminDashboard() {
  ...
  const handleTabChange = (tabId) => {
    setActiveTab(tabId);
    setSidebarOpen(false); // Tutup sidebar otomatis di mobile setelah klik menu
  };
    ...
    <div className="min-h-screen bg-gray-100 flex font-sans text-[#222222]">
      ...
      `}>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto h-[calc(100vh-140px)]">
          {tabs.map(tab => (
            <button 
              ...
              ${activeTab === tab.id ? 'bg-[#222222] text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
            >
        ...
        <div className="p-4 border-t absolute bottom-0 w-full bg-white">
          <button onClick={() => supabase.auth.signOut().then(() => window.location.reload())} className="w-full text-left px-4 py-2 text-red-500 text-sm font-bold flex items-center gap-2 hover:bg-red-50 rounded-lg transition">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 md:p-8 min-h-[500px]">
                 ...
                 <div>
                    ...
                    <div className="bg-white border rounded-lg overflow-hidden">
                      <ReactQuill 
                        ...
                        onChange={val => setData({...data, hero: {...data.hero, name: val}})}
                      />
                    </div>
                    <p className="text-[10px] text-gray-400 mt-1">*Contoh: "Halo, <strong>Annisa</strong> di sini!"</p>
                 </div>
            ...
            {activeTab === 'skills' && (
               <div className="space-y-8">
                  ...
                  <div>
                    ...
                    <textarea className="w-full p-4 border rounded-xl bg-gray-50 text-sm" rows={4} value={data.skills?.description} onChange={e => setData({...data, skills: {...data.skills, description: e.target.value}})} />
                  </div>
               </div>
            )}
            ...
            {activeTab === 'portfolio' && (
              <div className="space-y-10">
                <DynamicListEditor title="Instagram Content" items={data.portfolio?.ig_posts} onChange={v => setData({...data, portfolio: {...data.portfolio, ig_posts: v}})} schema={[{ key: 'label', label: 'Judul Konten' }, { key: 'role', label: 'Peran' }, { key: 'reels_url', label: 'Link Instagram' }, { key: 'video_url', label: 'Upload Video', type: 'media', mediaType: 'video' }]} />
                <DynamicListEditor title="Videography Project" items={data.portfolio?.video_projects} onChange={v => setData({...data, portfolio: {...data.portfolio, video_projects: v}})} schema={[{ key: 'label', label: 'Judul Video' }, { key: 'role', label: 'Peran' }, { key: 'video_url', label: 'File Video', type: 'media', mediaType: 'video' }]} />
                <DynamicListEditor title="Digital Characters" items={data.portfolio?.digital_characters} onChange={v => setData({...data, portfolio: {...data.portfolio, digital_characters: v}})} schema={[{ key: 'name', label: 'Nama Karakter' }, { key: 'image', label: 'Gambar Karakter', type: 'media' }]} />
                <DynamicListEditor title="Packaging Design" items={data.portfolio?.packaging} onChange={v => setData({...data, portfolio: {...data.portfolio, packaging: v}})} schema={[{ key: 'title', label: 'Nama Produk' }, { key: 'description', label: 'Deskripsi', type: 'textarea' }, { key: 'image', label: 'Desain Packaging', type: 'media' }]} />
                <DynamicListEditor title="Photography" items={data.portfolio?.photos} onChange={v => setData({...data, portfolio: {...data.portfolio, photos: v}})} schema={[{ key: 'title', label: 'Judul Foto' }, { key: 'description', label: 'Keterangan', type: 'textarea' }, { key: 'image', label: 'File Foto', type: 'media' }]} />
              </div>
            )}
