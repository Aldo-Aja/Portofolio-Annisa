...
// --- KOMPONEN: IMAGE/VIDEO UPLOADER (SUDAH DIPERBAIKI) ---
const MediaUploader = ({ currentUrl, onUpload, label, type = "image" }) => {
  ...
  return (
    <div className="mb-3">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {currentUrl && type === "image" && (
          <img
            src={currentUrl}
            alt="Preview"
            className="w-16 h-16 rounded-lg object-cover border bg-gray-50 shadow-sm"
          />
        )}
        ...
        {currentUrl && type === "video" && (
          <video 
            src={currentUrl} 
            className="w-24 h-16 rounded-lg object-cover border bg-black shadow-sm"
            muted 
          />
        )}
      ...
      {currentUrl && (
        <p className="mt-1 text-[10px] text-gray-400 truncate max-w-[200px]">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {items?.map((item, idx) => (
        <div key={idx} className="flex gap-2">
          <input
            className="flex-1 p-2.5 border rounded-lg text-sm focus:border-black outline-none shadow-sm"
            value={item}
            onChange={(e) => {
  ...
  return (
    <div className="bg-gray-50 p-4 md:p-6 rounded-xl border mb-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          onClick={addItem}
          className="text-xs bg-black text-white px-3 py-2 rounded-lg flex items-center gap-1 hover:bg-gray-800 shadow-sm active:scale-95 transition"
        >
      ...
      <div className="space-y-4">
        {items?.map((item, idx) => (
          <div
            key={idx}
            className="bg-white p-4 rounded-xl border shadow-sm relative group"
          >
            <button
                ...
                >
                  {field.type === "media" ? (
                    ...
                    <div>
                      ...
                      <textarea
                        className="w-full border p-3 rounded-lg text-sm focus:border-black outline-none focus:ring-1 ring-black/10"
                        rows={2}
                    ...
                    <div>
                      ...
                      <input
                        className="w-full border p-3 rounded-lg text-sm focus:border-black outline-none bg-transparent"
                        value={item[field.key] || ""}
                        onChange={(e) =>
                          updateItem(idx, field.key, e.target.value)
                        }
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-[#F4ECE6] px-4">
        <form
          ...
          className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm space-y-4 text-center"
        >
      ...
      <main className="flex-1 w-full md:w-auto p-4 md:p-8 pt-20 md:pt-8 overflow-y-auto h-screen">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 sticky top-0 md:top-0 bg-gray-100 py-4 z-20 backdrop-blur-sm bg-opacity-90 gap-4 md:gap-0">
            <div>
              <h2 className="text-2xl md:text-3xl font-serif font-bold capitalize text-gray-900">
                {activeTab} Editor
              </h2>
            ...
            <button
              onClick={handleSave}
              className="w-full md:w-auto bg-blue-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:bg-blue-700 hover:shadow-xl transition transform active:scale-95 flex items-center justify-center gap-2"
            >
              ...
              <div className="space-y-8 max-w-2xl">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 mt-1">
                    *Contoh: "Halo, <strong>Annisa</strong> di sini!"
                  </p>
                </div>
            ...
            {activeTab === "skills" && (
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <textarea
                    className="w-full p-4 border rounded-xl bg-gray-50 text-sm"
                    rows={4}
